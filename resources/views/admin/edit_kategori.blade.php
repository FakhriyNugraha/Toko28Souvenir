...
<body class="bg-pink-100 min-h-screen flex">
    ...
    <div class="flex-1 flex flex-col w-full">
        ...
        <header class="bg-white flex justify-between items-center px-6 py-4 shadow-md">
            <h1 class="text-xl font-bold text-pink-600">Edit Kategori</h1>
        ...
        <section class="p-6 mt-6 bg-white rounded-lg shadow-md">
            <form action="{{ route('admin.update_kategori', $kategori->id) }}" method="POST">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" id="nama" name="nama" value="{{ $kategori->nama }}" class="w-full p-4 mt-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-pink-500" required>
                </div>
                <button type="submit" class="bg-pink-600 text-white px-6 py-3 rounded-md hover:bg-pink-700 transition duration-200">Perbarui Kategori</button>
            </form>
        </section>
    </div>
